...
2. In the Search for Hardware search box, type in the selected board <b>FRDM-MCXW71</b>. Click on the board to select it.
3. On the right hand side you will see the option to build the SDK for the FRDM-MCXW71. Click on this button to add the middleware needed.
4. To build the SDK, let’s <b>Select All</b> for the middleware available. Then scroll down and press <b>Download SDK</b>. 
5. Once downloaded, drag and drop to the <b>Installed SDKs</b> window in the MCUXpresso IDE.
6. Locate the <b>sb3 image file</b> in the SDK package:<br> 
   `middleware\wireless\ieee-802.15.4\bin\k32w1_mcxw71\k32w1_mcxw71_nbu_ble_15_4_dyn_matter.sb3`
7. Copy the sb3 file into the bin folder of the MCU-Link installation directory- <br>
     `<MCU-Link Install Dir>\bin\`
8.  Plug MCXW71 to the USB port
9.  Use the <b>blhost tool</b> to flash the NBU image, open a command prompt and call the program script:<br>
`Windows users: Check the COM port number in Device Manager before running the command.`
```
    cd <MCU-Link Install Dir>\bin\
    blhost.exe -p COM27 receive-sb-file <sb3 file>
```
...
For detailed instructions on adding the FRDM-RW612 to the chip-tool app, please refer to the following link: [FRDM RW612 Matter light & switch app plus Thread Board Router solution](https://github.com/nxp-appcodehub/ap-frdm-rw612-light-switch-otbr-combo#step6).
